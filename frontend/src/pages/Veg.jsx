...
const Veg = () => {
  ...
  const fetchVegRecipes = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await recipeService.getRecipesByCategory('Veg');
      setRecipes(response.recipes || []);
    } catch (err) {
      setError('Failed to load vegetarian recipes');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: 'var(--color-secondary)' }}>
      <div className="relative overflow-hidden py-14 mb-6" style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.08) 0%, rgba(15,15,15,0) 60%)' }}>
        <div className="container-custom">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-4xl font-extrabold text-white" style={{ fontFamily: 'Poppins, sans-serif' }}>Vegetarian Recipes</h1>
          </div>
          <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>Fresh, healthy and delicious plant-based dishes</p>
        </div>
      </div>
      ...
      <div className="container-custom pb-16">
        ...
        {recipes.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-xl font-semibold text-white mb-2">No vegetarian recipes found</p>
